...
Au cours des différents travaux réalisés au cours de ma thèse, nous avons pu constater la puissance des technologies de séquençage haut-débit. En effet, en seulement ... [TODO] ans, celles-ci ont permis l'identification de 5 nouveaux gènes impliqués dans des phénotypes d'infertilité au sein de notre laboratoire. Ces résultats sont cependant à relativiser puisque pour plusieurs de nos patients, aucun candidat n'a pu être identifié. Plusieurs raisons peuvent expliquer cela. Tout d'abord, au cours des analyses décrites dans ces manuscrits nous nous concentrons uniquement sur les SNPs et les indels. Cependant de nombreux logiciels tel que ExomeDepth [@Plagnol2012], CoNIFER [@Krumm2012] ou encore ExomeCNV [@Sathirapongsasuti2011] permettent de détecter des CNVs à partir de données WES et / ou WGS. Les stratégies de prédictions de ces logiciels pouvant être extrêmement différents (**Figure : **\@ref(fig:pictcnvdetection)), le profil des CNVs détectés ou non le sera aussi [@Zhao2013 ; @Guo2013]. Ainsi, dans des analyses non décrites dans ce manuscrit, j'ai pu chercher à identifier des CNVs à partir de nos données d'exome à l'aide du logiciel ExomeDepth [@Plagnol2012]. Cette approche a été extrêmement concluante puisqu'elle a permis d'identifier une délétion homozygote sur le gène *WDR66* chez 7 de nos patients pour lesquels aucun candidat n'avait été alors identifié. Ces délétions ont ensuite pu être confirmées par PCR et la caractérisation de ce gène est actuellement en cours au sein de notre équipe. Au vu de cette réussite, il est désormais prévu d'intégrer ce genre d'analyse de manière automatique et systématique au sein de notre pipeline.  
...
fig_description <- "**A** : Cette stratégie, permet de prédire des CNVs à partir des alignements discordant des deux *ends* d'un même *read*, c'est à dire en répertoriant les *reads* pour lesquels la distance séparant les deux *ends* après l'alignement est significativement supérieure à la taille moyenne de l'insert. **B** : La méthode *split-read* se base sur les *reads* s'alignant de manière partielle sur plusieurs régions génomiques. **C** : L'approche *read depth* compare la couverture observée sur plusieurs région génomique pour prédire des CNVs. **D** : Cette méthode effectue un assemblage *de novo* (sans utilisé de génome de référence) ; les résultats de l'assemblage appelés *contigs* sont comparés au génome de référence a posteriori pour détecter les CNVs. **E** : Cette méthode combine les approches **A** et **C**."
...
Après avoir fait ses preuves dans la recherche, le séquençage NGS cherche aujourd'hui à percer dans le domaine du diagnostique clinique. On peut alors prédire sans trop prendre de risques que ces technologies seront d'ici quelques années incontournables dans ce domaine. Il est néanmoins légitime de se demander dès aujourd'hui quelle sera son efficacité. En effet, en se basant sur les données de nos `r nb_run_conclu` patients non apparentés souffrant du syndrome MMAF on peut s'attendre, à obtenir entre `r min_var_per_run` et `r max_var_per_run` variants par patients avec un coefficient de variation de  `r coef_variation`% (avec $C_v = \frac{\sigma}{\mu}$ où $\sigma$ est l'écart-type et $\mu$ la moyenne). Ces chiffres sont obtenus en filtrant de notre ensemble de variants ceux ayant une fréquence $\ge$ `r maf_cutoff` dans les bases de données publiques ainsi qu'en retirant les variants introniques, synonymes et ceux impactant les séquences UTRs. On constate alors que parmi l'ensemble de ces variants `rn_var_unique` d'entre eux (`r p_var_unique`%) sont "individuels", c'est à dire porté uniquement par un seul des patients (**Figure : **\@ref(fig:plotvarperimpact) - **A**). De même on peut observer que chacun de nos patients est porteur d'environs `r median_high_per_run` variants tronquants parmi lesquels environs `r median_homo_high_per_run` le sont à l'état homozygote (**Figure : **\@ref(fig:plotvarperimpact) - **B**). La priorisation de certains gènes par des outils tel que MutaScript permettra alors d'orienter les analyses vers les gènes les plus prometteurs. De même la recherche de variants dans des panels de gène permettra égallement de cibler les recherches sur les gènes déjà connus comme étant lié au phénotype en question. Ainsi, au vu de nos réultats dans le cadre de patients atteins du syndrome MMAF, la recherche de variants dans les gènes *DNAH1*, *CFAP43*, *CFAP44* et *WDR66* permettrait d'obtenir un diagnostique positif dans environs `r (((5 + 10 + 6 + 7) / nb_run_conclu) * 100) %>% round(0)`% des cas. On peut dès lors s'attendre que les recherches futures vont permettre d'agrandir cette liste de gène cibles améliorant ainsi l'effeciacité du diagnostique.      
