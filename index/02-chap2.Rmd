...
fig_name <- "Observation au microscope à balayage de spermatozoïdes"
fig_description_a <- "**A** : Spermatozoïde normal."
fig_description_b <- "**B** : Spermatozoïde globozoocéphale. TODO :: changer les photos avec celles sur lesquelles on voit l'acrosome colorés)" 
...
Ce mécanisme de NAHR devrait, en théorie, engendrer la formation de plus d'allèles délétés que d'allèles dupliqués puisque les cas présentés en figures \@ref(fig:pictnahr) - **2** et \@ref(fig:pictnahr) - **3** induisent la formation d'un allèle délété tandis que seul le cas \@ref(fig:pictnahr) - **3** forme un allèle dupliqué [@Liu2012]. Cependant, les données mises à disposition par la base de données [*Database of Genomic Variants*](http://dgv.tcag.ca/dgv/app/home) (DGV) [@MacDonald2014] indiquent un excès de duplication puisque sur un total de 6575 individus analysés, 83 duplications et de 26 délétions hétérozygotes ont été observées pour le locus de *DPY19L2*.  
...
```{r plotborn, fig.height=3.6, fig.cap='(ref:plotborn)',fig.scap=fig_name, cache=TRUE, dependson="dfborn"}
...
fig_description <- paste0("Après ", n_birth_without_last, " naissances, ",  n_female_all - n_female_last, " souriceaux mâles sont nés pour seulement ", n_male_all - n_male_last, " femelles, ainsi le test du $\\chi^2$ comparant ces valeurs donne une p-valeur de ", p_test_without_last, ".")
...
```{r plotmolfunction, cache=TRUE, dependson="dfmolfunction", warning=FALSE, message=FALSE, fig.height=8, fig.width=6.5, fig.cap='(ref:plotmolfunction)', fig.scap=fig_name}
