...
```{r expspink2guf1, include=FALSE, message=FALSE, warning=FALSE}

exp_spink2_guf1 <-  retrieve_exp(gene = c("spink2","guf1"))
...
ggplot(exp_spink2_guf1, aes(Tissue, `%Expression`)) + 
  geom_col(aes(fill = SYMBOL), position = position_dodge()) + 
  ylab("% of RPKM") +
  scale_y_continuous(labels = percent) + 
  xlab("") + 
  scale_fill_brewer(name = "Gene", palette = "Set1") +
  coord_flip() +
...
genes_mmaf3 <- df_remaining_gene %>% filter(Familly == "MMAF3") %>% collect() %>% .[["SYMBOL"]]
exp_genes_mmaf3 <- retrieve_exp(gene = genes_mmaf3)
...
ggplot(exp_genes_mmaf3, aes(Tissue, `%Expression`)) + 
  geom_col(aes(fill = SYMBOL), position = position_dodge()) + 
  ylab("% of RPKM") +
  scale_y_continuous(labels = percent) + 
  xlab("") + 
...
df_exp_candidat <- retrieve_exp(gene = candidat_gene)
...
ggplot(df_exp_candidat, aes(Tissue, `%Expression`)) + 
  geom_col(aes(fill = SYMBOL), position = position_dodge()) + 
  ylab("% of RPKM") +
  scale_y_continuous(labels = percent) + 
  xlab("") + 
