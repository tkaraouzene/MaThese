...
df_strand_all <-  df_call_fam %>%
  ...
  group_by(RUN) %>%
  mutate(P = N / sum(N),
         IS_RETAINED = if_else(STRAND == "NS","No","Yes"))
...
Pour une d'entre elle, un cadidat potentiel a pu être mis en évidence avec le gène *TGIF2* et notre équipe travaille actuellement sur la caractérisation de ce gène afin de savoir s'il peut effectivement expliquer le phénotype MMAF de cette famille (**Figure : **\@ref(fig:plotremaininggenes)).     
...
```{r bigmmafannotate, cache=TRUE}

bigmmaf_annotate_file <- "results/bigmmaf_annotate_variants.rds"

if (!file.exists(bigmmaf_annotate_file)) {

  df_bigmmaf_annotate <- df_bigmmaf_call %>%
    filter(grepl("zygous", CALL)) %>% 
    select(-STRAND) %>%
    variant_join(fread("data/all_variants_vep81.csv", sep = "\t", header = TRUE, showProgress = FALSE, verbose = FALSE), mode = "left") %>% 
    select(-Allele, -Gene, -Feature_type, -(BIOTYPE:HGNC_ID), -(CADD_RAW:ExAC_AF_OTH), -(ExAC_AF_AMR:ExAC_AF_SAS)) %>%
  ...
  saveRDS(df_bigmmaf_annotate, bigmmaf_annotate_file)
  
} else {
  
  df_bigmmaf_annotate <- readRDS(bigmmaf_annotate_file)
