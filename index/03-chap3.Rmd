...
df_variants_mmaf <- fread("data/genotype_MMAF.csv", sep = "\t", header = TRUE, showProgress = FALSE, verbose = FALSE) %>%
  gather(RUN, CALL, starts_with("Ghs")) %>%
  filter(CALL %in% c(2, 3, "b", "c")) %>%
  mutate(TYPE = if_else(stri_length(REF) == stri_length(ALT), "SNV", "Indel"), 
         STRAND = if_else(CALL %in% c(2, 3), "Both_strand", "Single_strand"),
...
# df_mmaf_annotated <- inner_join(df_variants_mmaf,df_all_coding_variants, by = c("CHR", "POSITION", "REF", "ALT"))

df_not_mmaf <- fread("data/genotype_not_MMAF.csv", sep = "\t", header = TRUE, showProgress = FALSE, verbose = FALSE) %>%
  gather(RUN, CALL, starts_with("Ghs")) %>%
  filter(CALL %in% c(3, "c")) %>%
  select(-CALL)
...
df_filtter_mmaf <- df_variants_mmaf %>%
  inner_join(df_all_coding_variants, by = c("CHR", "POSITION", "REF", "ALT")) %>%
...
df_passing_filter_mmaf_genes <- distinct(df_filtter_mmaf, SYMBOL,CHR,POSITION,REF,ALT) %>%
       select(Chromosome = CHR, Position = POSITION,`Reference allele` = REF,`Alterated allele` = ALT,Gene=SYMBOL)

nb_run_per_passing_gene <- df_filtter_mmaf %>%
  distinct(SYMBOL,RUN) %>%
  group_by(SYMBOL) %>%
  summarise(N = n())
...
df_variants_mmaf_nb_view <- df_variants_mmaf %>%
  filter(!is.na(REF)) %>%
  distinct(CHR,POSITION,REF,ALT,TYPE,RUN) %>%
  group_by(CHR,POSITION,REF,ALT,TYPE) %>%
  summarise(N = n(),TIME = "Before filtering")

df_var_mmaf <- df_filtter_mmaf %>% distinct(CHR,POSITION,REF,ALT,TYPE,RUN) %>% group_by(TYPE,RUN) %>% summarise(N = n())

df_nb_view <- df_filtter_mmaf %>% distinct(CHR,POSITION,REF,ALT,TYPE,RUN) %>% group_by(CHR,POSITION,REF,ALT,TYPE) %>% summarise(N = n(), TIME = "After filtering")
# 
