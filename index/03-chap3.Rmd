...
df_bigmmaf_genes2 <- df_bigmmaf_filter2  %>% group_by(SYMBOL, EVIDENCE, LOCALISATION) %>% distinct(RUN) %>% mutate(N_RUN = n()) 
...
Ensuite, en raison du grand nombre de gènes ayant passé les filtres et afin de nous concentrer en priorité ceux ayant la plus forte probabilité d'être impliquer dans le phénotype MMAF, nous avons étudiés les gènes sur lesquels plusieurs patients portaient au moins un variant ayant passé l'ensemble des filtres. Ainsi, nous avons obtenu une liste de `r n_gene_more1` gènes dont `r n_gene_2` (soit `r p_gene_2` %) étaient retrouvés variants chez uniquement 2 patients (**Figure : **\@ref(fig:plotfilterbigmmaf2)). 

Afin de nous orientés dans nos recherche, nous avons utilisés une liste de gènes annoté comme faisant partie du cilliome humain. cette liste recansse ... gènes et les classes en fonction de la qualité de la preuve permettant de définir le gène comme appartenant au cilliome. Ainsi, ... de nos ... gènes retenus fesaient partis de cette liste. Il faut tout de même noter que bien que cette liste soit un bon outil pour orienté les recherches et priosiser certains gènes, elle ne peut constituer un critère suffisant pour filtrer le gènes n'en faisant pas partie. Par exemple le gène *DNAH1*, de part son expression ubiquitaire n'a pas été intégré à cette liste, or on connait desormais son implication dans le phénotype MMAF.  

<!-- \newpage -->

```{r plotfilterbigmmaf2, fig.cap=fig_info,fig.scap=fig_name, cache=TRUE}
...
pA <- ggplot(df_bigmmaf_genes_distinct2 %>% 
         filter(!is.na(EVIDENCE)), aes(EVIDENCE)) +
...
pB <- ggplot(df_bigmmaf_genes_resume2 %>% ungroup () %>% mutate(EVIDENCE = if_else(is.na(EVIDENCE), "Not in the list", EVIDENCE)), aes(N_RUN, N_GENE)) + 
  geom_bar(aes(fill = EVIDENCE), stat = "identity") +
  # facet_wrap(~EVIDENCE, scales = "free") +
  # geom_text(aes(label =  N_GENE), size = 3, position = position_dodge(width = .9), hjust = -.25) +
  scale_x_reverse(breaks=c(2:15)) + 
  coord_flip() + 
  scale_fill_brewer(palette = "Set1") + 
  theme(legend.position = c(.79,0.315))


plot_grid(pA, pB, labels = c("A","B"), ncol = 1)

# scale_x_discrete(limits = c("Strong evidence from previous studies")), 
#                                "Weak evidence from previous studies",
#                                "No evidence from previous studies"))
# 
# 
...
Cette analyse nous a ainsi permis de mettre en évidence ... nouveaux acteurs potentiels pour le phénotype MMAF impliquant ... de nos patients.Ayant conservé uniquement les variants homozygotes au cours de nos filtre, nous avons comme précédemment (introduire le truc u niveau de dnah1) chercher à savoir si parmis les individus restant de la cohorte, certains ne seraient pas hétérozygotes composites.
...
L'étude de ces gènes nous a tout d'abord permis d'identifier deux gènes : le gène *WDR96* (récemment renommé *CFAP43*) et le gène *WDR52* récemment renommé *CFAP44*). Ces gènes ont respectivement été retrouvés mutés chez `r n_run_homo_wdr96` et `r n_run_homo_wdr52` patients. En plus du nombre important de patients portant une mutation tronquante sur ces gènes (**Table : **\@ref(tab:tabwdr96) et \@ref(tab:tabwdr52)), tout deux sont retrouvés comme étant sur-exprimés dans le testicule dans les bases de donnée publiques. De même, ces deux gènes avaient déjà été décrits dans la litterature comme étant impliqués dans la structure et / ou le fonctionnement du flagelle spermatique [@Ivliev2012]. Ces informations ont ainsi fait de ces deux gènes des candidats idéaux pour expliquer le phénotype d'au moins `r n_run_homo_wdr96 + n_run_homo_wdr52 - 1` de ces `r n_run_homo_wdr96 + n_run_homo_wdr52` individus. En effet, au vu de l'incertitude de l'effet de la mutation impactant le patient ..., c'est à dire un variant intronique situé dans la région d'épissage, la causalité de ce variant dans le phénotype du patient est elle aussi incertaine.     

Cette étude nous a permis égallement de relever `r n_run_homo_fsip2` patients présentants des mutations dans le gènes *FSIP2* (**Table : **\@ref(tab:tabfsip2). Comme pour les gènes *WDR96* et *WDR52*, ce gène présente une forte et quasi exclusive expression testiculaire. De plus, une équipe a démontré en 2003 l'implication de ce gène dans la structure de la gaine fibreuse su flagelle spermatique [@Brown2003] faisant égallement de ce gène un excellent candidat dans l'explication du phénotype MMAF de ces patients.   

Après avoir identifié la cause génétique du phénotype MMAF de ces `r n_run_homo_dnah1 + n_run_homo_wdr96 + n_run_homo_wdr52 + n_run_homo_fsip2` patients (`r n_run_homo_dnah1` patients *DNAH1*, `r n_run_homo_wdr96` *WDR96*, `r n_run_homo_wdr52` *WDR52* et `r n_run_homo_wdr52` *FSIP2*) grâce à des variants homozygotes, nous avons chercher l'ensemble des patients pouvant être hétérozygotes composites sur l'un de ces 4 gènes, . 

Ainsi, cette stratégie nous a permis d'identifier `r n_run_het_dnah1` nouveaux patients présentant des mutation sur le gène *DNAH1*, `r n_run_het_wdr96` autres sur WDR96 et `r n_run_het_fsip2` sur *FSIP2*. Aucun fut trouvé avec au moins deux mutations hétérozygotes sur le gène *WDR52*. Il faut tout de même noter la limite de cette approche dûe au fait de la non connaissance du phasage des variants, c'est à dire qu'il nous est impossible de déterminer si les deux variants sont situés sur le même allèle ou bien sur deux allèles différents. 
...
# 
# df_all_runs <- fread("data/exome_runs.csv") %>%
#   select(RUN = `Magic ID`, Patient, PHENOTYPE = Pathologie) %>%
#   right_join(df_bigmmaf_filter_homo_het, by = "RUN") %>%
#   filter(SYMBOL == "WDR96" |
#            SYMBOL == "DNAH1" |
#            SYMBOL == "FSIP2" |
#            SYMBOL == "WDR52" |
...
#   distinct(CHR, POSITION, REF, ALT, CALL, SYMBOL, Consequence) %>%
#   arrange(SYMBOL, desc(CALL), RUN)
# # 
# # df_all_runs %>% write.table("~/Bureau/gene_MMAF_31052017.csv", quote = FALSE, sep = "\t")
#  df_all_runs %>% View
# # 
...
# 
# 
# df_bigmmaf_genes_homo_het <- df_bigmmaf_filter_homo_het %>% ungroup() %>% select(SYMBOL, RUN, CALL) %>% group_by(SYMBOL, CALL) %>% distinct(RUN) %>% mutate(N_RUN = n()) %>% group_by(SYMBOL) %>% filter(n_distinct(RUN) > 1)
# 
# df_bigmmaf_genes_distinct_homo_het <- df_bigmmaf_genes_homo_het %>% distinct(CALL, N_RUN) %>% mutate(S = sum(N_RUN))
# df_bigmmaf_genes_resume_homo_het <- df_bigmmaf_genes_distinct_homo_het %>% distinct(N_RUN) %>% group_by(N_RUN) %>% summarise(N_GENE = n()) 
# 
# 
# 
# ggplot(df_bigmmaf_genes_resume_homo_het, aes(N_RUN, N_GENE)) + 
#   geom_bar(aes(fill = CALL), stat = "identity") +
#     geom_text(aes(label =  N_GENE), size = 3, position = position_dodge(width = .9), hjust = -.25) +
#   # scale_x_continuous(breaks = c(2:15)) +
#   scale_x_reverse( breaks=c(2:50)) + 
#   coord_flip() + 
#    scale_fill_brewer(palette = "Set1")
# 
# plotly::ggplotly(ggplot(df_bigmmaf_genes_distinct_homo_het, aes(reorder(SYMBOL, -S),N_RUN)) + 
#   geom_bar(stat = "identity", aes(group = S, fill = CALL)))
...
<!-- Comme précédemment, la cause génétique expliquant le phénotype de ces `r n_found` patients ayant été identifiée, leurs données ont été retirées des analyses ulterieure réduisant ainsi nos listes à `r n_distinct_var2` variants et `r n_distinct_gene2` gènes distincts.    -->
