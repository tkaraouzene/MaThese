...
Dans cette partie, je me concentre sur l'analyse bioinformatique des résultat du séquençage exomique effectué entre `r min(df_seq$YEAR)` et `r max(df_seq$YEAR)` de `r sum(df_seq$NB)` individus infertiles provenant de `r nb_fam` familles différentes. Parmi celles-ci, `r nb_pheno` phénotypes différents ont été observés : 
...
tab.name <- "Tableau des gènes ayant passé l'ensemble des filtres pour la famille AZ"
df_res_az <- df_remaining_gene %>% filter(Familly == "AZ") %>% select(Gene = SYMBOL) 

kable(df_res_az , caption = tab.name)

```

```{r tablegeneff}

tab.name <- "Tableau des gènes ayant passé l'ensemble des filtres pour la famille FF"
df_res_ff <- df_remaining_gene %>% filter(Familly == "FF") %>% select(FamiGene = SYMBOL)

kable(df_res_ff, caption = tab.name)

```

```{r tablegenemmaf1}

tab.name <- "Tableau des gènes ayant passé l'ensemble des filtres pour la famille MMAF1"
df_res_mmaf1 <- df_remaining_gene %>% filter(Familly == "MMAF1") %>% select(Gene = SYMBOL) 

kable(df_res_mmaf1, caption = tab.name)

```

```{r tablegenemmaf2}

tab.name <- "Tableau des gènes ayant passé l'ensemble des filtres pour la famille MMAF2"
df_res_mmaf2 <- df_remaining_gene %>% filter(Familly == "MMAF2") %>% select(Gene = SYMBOL)

kable(df_res_mmaf2 , caption = tab.name)

```

```{r tablegenemmaf3}

tab.name <- "Tableau des gènes ayant passé l'ensemble des filtres pour la famille MMAF3"
df_res_mmaf3 <- df_remaining_gene %>% filter(Familly == "MMAF3") %>% select(Gene = SYMBOL) 

kable(df_res_mmaf3 , caption = tab.name)

```

```{r tablegenemmaf4}

tab.name <- "Tableau des gènes ayant passé l'ensemble des filtres pour la famille MMAF4"
df_res_mmaf4 <- df_remaining_gene %>% filter(Familly == "MMAF4") %>% select(Gene = SYMBOL) 

kable(df_res_mmaf4, caption = tab.name)
