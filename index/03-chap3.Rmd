...
```{r filterbigmmaf, dependson="bigmmafcall", cache=TRUE}
...
df_bigmmaf_filter2 <- df_bigmmaf_filter %>% 
  filter(!(RUN %in% run_found)) %>%
    mutate(EVIDENCE = if_else(is.na(EVIDENCE), "Not in the list",
           ...
           SYMBOL = if_else(SYMBOL == "WDR52", "CFAP44", 
...
Cependant, comme nous l'avons vu précédemment, bien que la stratégie consistant à filtrer l'ensemble des variants hétérozygotes chez les individus apparentés, celle-ci peut dans le cas d'individus non apparentés entrainer le filtrage du / des variants responsables du phénotype. Ces pourquoi nous avons ensuite cherché des patients potentiellement composites, c'est à dire des patients portant au moins deux allèles **différents** mutés sur le même gène. Pour cela, nous avons recenser l'ensemble des patients portant au moins deux variants hétérozygotes différents sur le gène *DNAH1*. Cette stratégie nous a ainsi permis de découvrir `r n_run_het_dnah1` nouveaux patients pour lesquels le gène *DNAH1* serait responsable du phénotype MMAF. Il faut tout de même noter, qu'en raison de la non connaissance du phasage des variants dans le cas de séquençage WES, le génotype de ces `r n_run_het_dnah1` patients est à confirmer de sorte à être certains que les deux variants se situent bien sur deux allèles différents de *DNAH1*, un sur celui provenant du père et un autre sur celui provenant de la mère.   

Néanmoins, les mutations du gène *DNAH1* seraient ainsi responsables du phénotype MMAF de `r p_run_tot_dnah1` % des individus de notre cohorte. Bien que ce pourcentage soit en deçà des 40% (TODO: à confirmer!) observés dans notre étude précédente [@BenKhelifa2014], ces résultats tendent à confirmer le rôle primordial de la protéine DNAH1 dans la structure du flagelle et l'implication majeure du gène *DNAH1* dans le phénotype MMAF.   
...
```{r tabdnah1, dependson="removednah1", cache=TRUE}
...
\newpage
```{r expdnah1, fig.cap=fig_info, fig.height=7, fig.scap=fig_name, cache=TRUE}

fig_name <- "Expression tissulaire du gène *DNAH1*"
fig_info <- paste0(fig_name, " : D'après les données du Illumina BodyMap")

plot_expression("dnah1", "percent", TRUE) +
  xlab("") + 
  theme(legend.position = c(1,.5),
        legend.justification = c(1,.5))
...
df_bigmmaf_gene_grp1 <- df_bigmmaf_filter2 %>% 
  ...
  mutate(N_RUN = max(N_RUN)) %>% 
  group_by(RUN, SYMBOL) %>%
  distinct(Consequence, SIFT, PolyPhen, EVIDENCE)

```

```{r vargrp1moderate, dependson="dfgrp1moderate", cache=TRUE}

n_gene_grp1 <- df_bigmmaf_gene_grp1 %>% ungroup() %>% distinct(SYMBOL) %>% nrow
run_grp1 <- df_bigmmaf_gene_grp1 %>% ungroup() %>% distinct(RUN) %>% collect() %>% .[["RUN"]]
...
df_grp1_low <- df_bigmmaf_filter_het2 %>% 
  ...
  mutate(N_RUN = max(N_RUN)) %>% 
  group_by(RUN, SYMBOL) %>%
  mutate(PolyPhen = if_else(grepl("damaging", PolyPhen), "damaging", PolyPhen))

```

```{r dfresumegrp1, dependson=c("dfgrp1high","dfgrp1moderate","dfgrp1low"), cache=TRUE}

df_bigmmaf_resume_grp1 <- bind_rows(df_distinct_cil_high %>%
                                      select(-EVIDENCE) %>%
                                      mutate(TRUST = "High"),
                                    
                                    df_bigmmaf_gene_grp1 %>%
                                      select(-EVIDENCE) %>%
                                      group_by(SYMBOL) %>%
...
p_run_high_evidence <- df_bigmmaf_resume_grp1 %>% ungroup() %>% summarise(P = sum(High)/ sum(TOT)) %>% collect() %>% .[["P"]] * 100
...
```{r dfgrp2high, dependson="removednah1", cache=TRUE}
...
df_grp2_high <- df_bigmmaf_filter3 %>% 
  filter(IMPACT == "HIGH") %>%
  group_by(SYMBOL, Feature) %>%
  mutate(Consequence = sapply(strsplit(Consequence, "_variant", fixed = TRUE),"[[",1),
         Consequence = gsub("_", " ", Consequence),
         N_RUN = n_distinct(RUN)) %>% 
  filter(N_RUN > 1) %>% group_by(SYMBOL) %>% mutate(N_RUN = max(N_RUN)) %>% 
  group_by(RUN, SYMBOL) %>%
  distinct(Consequence, N_RUN) %>%
  filter(SYMBOL != "hsa-mir-150")

df_distinct_gene_grp2_high <- df_grp2_high %>% ungroup() %>% distinct(SYMBOL, N_RUN)
# df_bigmmaf_grp2 %>% View
# df_distinct_gene_grp2 %>% View
...
gene_grp2 <- df_grp2_high %>% collect() %>% .[["SYMBOL"]]
...
df_grp2_low <- df_bigmmaf_filter_het3 %>% 
  ...
  mutate(N_RUN = n_distinct(RUN)) %>% 
  group_by(SYMBOL) %>% 
  distinct(CHR, POSITION, REF, ALT, RUN, Feature, Consequence, N_RUN, SIFT, PolyPhen) %>% 
  inner_join(df_canonical_transcripts, by = "Feature")
...
df_bigmmaf_resume_grp2 <- bind_rows(df_grp2_high_reduce %>% summarise(TRUST = "High"),
                                    df_grp2_moderate %>% summarise(TRUST = "Moderate"),
                                    df_grp2_low %>% summarise(TRUST = "Low")) %>% 
...
Ensuite, comme précédemment, nous avons rechercher les patients portant des variants non tronquant sur ces 3 gènes ainsi que ceux portant au moins deux variants hétérozygotes tronquant ou non. Nous permettant ainsi d'identifier `r n_run_grp2_moderate + n_run_grp2_low` nouveaux patients. 
