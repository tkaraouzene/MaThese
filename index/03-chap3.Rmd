...
df_variants_b1_b2 <- fread("../../genotype_plcz.csv",sep = "\t", header = TRUE,showProgress = FALSE, verbose = FALSE) %>%
...
df_ctrl <- fread("../../genotype_not_fert_fail.csv",sep = "\t", header = TRUE,showProgress = FALSE, verbose = FALSE) %>%
  gather(RUN,CALL,starts_with("Ghs")) %>%
...
View(df_passing_filter_genes)


```


```{r plc_zeta_tabcommonvar}

tab.name <- "Comptage des variants communs à B1 et B2"

knitr::kable(group_by(df_filter_common_variants,
               TYPE,GENOTYPE) %>% 
