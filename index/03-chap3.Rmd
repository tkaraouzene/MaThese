...
plot_resume_analysis <- function(df) {
  ...
  df <- df %>% 
    gather(TRUST,NB, -SYMBOL, -TOT) %>%
    mutate(TRUST = factor(TRUST, levels = levels))
  
  break_max <- df$NB %>% max
  
  ggplot(df, aes(TRUST, NB)) +
...
kable(bind_rows(df_dnah1_high %>% mutate(TRUST = "High"),
      ...
      booktabs = TRUE,
      longtable = TRUE,
      caption = "Liste des patients portant au moins un variant homozygote ou deux variants hétérozygotes sur le gène *DNAH1*")
...
run_cil_high <- df_grp1_high %>% filter(EVIDENCE != "Not in the list") %>% ungroup() %>% distinct(RUN) %>% collect() %>% .[["RUN"]]
...
run_found <- c(run_found, run_cil_high, run_grp1_moderate)
...
4. **Étape n°4** : La cause génétique responsable du phénotype de ces `r n_run_cil_high + n_run_grp1_moderate` patients ayant été identifiés, nous avons comme précédemment avec les patients *DNAH1* retirer l'intégralité de leurs variants des analyses ulterieures afin de réduire à nouveau la liste des variants et des gènes à analyser facilitant ainsi les analyses. Ainsi, ... variants sur ... gènes.     

Pour résumer, cette première analyse nous a permis de mettre en évidence `r n_gene_grp1` gènes dont différents plusieurs preuves laissent supposer leur implication dans le phénotype MMAF de `r n_run_cil_high + n_run_grp1_moderate` de nos patients. Pamis ceux-ci, `r n_run_cil_high` portent un variant tronquant à l'état homozygote nous donnant ainsi une grande confiance dans le fait que le variant soit effectivement à l'origine du phénotype. Pour les `r n_run_grp1_moderate` autres patients, les variants faux-sens portés par ces patients laisse planer un doute, cependant, le fait que ........... (**Figure : **\@ref(fig:plotgrp1high) - **B**)


\newpage

```{r plotgrp1high, fig.height=7.2, dependson=c("vargrp1","localfunction"), fig.cap=fig_info,fig.scap=fig_name, cache=TRUE}
...
pA <- plot_expression(gene_cil_high, mode = "percent", flip = TRUE) + 
  xlab("") + 
  theme(legend.position = c(1,.5),
        legend.justification = c(1,.5))
...
Pour finir nous avons dans cette analyse 
