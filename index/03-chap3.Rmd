...
call_to_geno <- function(call) {
  ...
  plyr::mapvalues(call, from = c(in_homo,in_het,in_none), to = c(out_homo, out_het, out_none), warn_missing = FALSE)
...
# df_novogene <- fread("data/results_novogene_filtered_Flag.csv", sep = "\t", header = TRUE, showProgress = FALSE, verbose = FALSE, data.table = FALSE) %>% 
#   rename(CHR = CHROM, POSITION = POS) %>% 
#     filter(Gencode != ".") %>% 
#   mutate(NOVO_HGVSc = sapply(strsplit(AAChange, split=':', fixed=TRUE), function(x) (x[4])),
#          NOVO_HGVSp = sapply(strsplit(AAChange, split=':', fixed=TRUE), function(x) (x[5])),
#          Gencode = sapply(strsplit(Gencode, split=':', fixed=TRUE), function(x) (x[1]))) %>%
#   left_join(df_distinct_filter, by = c("CHR", "POSITION" , "REF", "ALT")) %>%
#   unnest(strsplit(AAChange, ",", fixed = TRUE)) %>% 
#   
#   View  
# 
#   df_bigmmaf_filter %>% View
#   
#   df <- data_frame(
#  a = list(c("a", "b"), "c","d"),
#  b = list(1:2, 3,4:5),
...
df_bigmmaf_filter_reduce <- df_bigmmaf_filter_high %>% 
  ...
  mutate(N_RUN = na.replace(N_RUN, 0),
         N_TOT = sum(N_RUN)) %>%
  filter(!(SYMBOL %in% c("RP6-206I17.1","hsa-mir-150","HLA-DRB5"))) 
...
L'application de de notre pipeline d'analyse nous a permi d'obtenir une liste de ... variant distincts avec une moyenne de ... par patients (insert fig) sur lesquels nous avons appliqués les filtres précédemments décrits. Ainsi, (répéter les filtres utilisés). L'application de ces filtres nous a ainsi permis d'obtenir une liste "restreinte" de ... de SNV et ... indels impactant ... gènes différents. Au vu du nombre important de gène restant et afin d'étudier en priorité ceux pouvant expliquer le phénotype d'un maximum de patients nous avons, comme précédemment, limité nos premières recherches aux gènes sur lesquels **au moins 2 patients portaient un variant tronquant** réduisant à nouveaux cette liste à ... gènes différents. 

À l'issus de ces résultats nous nous somme dans un premiers temps concacrés à la caractérisation des gènes *CFAP43* et *CFAP44*  
...
bind_rows(df_bigmmaf_filter %>% filter(SYMBOL %in% c("CFAP43", "CFAP44")),
                      df_bigmmaf_filter_het %>% filter(SYMBOL %in% c("CFAP43", "CFAP44"))) %>%
  filter(CANONICAL == "YES") %>%
  mutate(HGVSc = na.replace(HGVSc, "."),
         HGVSp = na.replace(HGVSp, "."),
         ExAC_AF = na.replace(ExAC_AF, "."),
  ...
  select(Patient = RUN, Gene = SYMBOL, Genotype = CALL, `HGVSc, HGVSp`, Consequence, ExAC = ExAC_AF, ESP, `1KG`) %>%
   kable(format = "latex",
          booktabs = TRUE,
          longtable = TRUE,
          caption = caption) %>%
