...
run_fam_mmaf <- c("Ghs62", "Ghs130")
run_all_fam <- c(run_fam_az, run_fam_ff, run_fam_mmaf)

df_fam_run <- data_frame(FAM = c("AZ","AZ","FF","FF","MMAF","MMAF"),
                     RUN = c(run_fam_az, run_fam_ff, run_fam_mmaf),
...
df_fam <- df_fam_run %>% group_by(FAM, PHENOTYPE, YEAR, PLACE) %>%
...
n_gene_mmaf1 <- df_remaining_gene %>% filter(Family == "MMAF") %>% distinct(SYMBOL) %>% nrow
...
4. **Famille MMAF** : À l'issue des filtres, ` n_gene_mmaf` gènes ressortaient chez ces deux frères : *MYH11* et *DNAH1*. Or, notre équipe ayant déjà établit le lien entre des mutations du gène *DNAH1* et le syndrome MMAF [@BenKhelifa2014] ce gène s'est révélé être un candidat idéal pour expliquer le phénotype de ces 2 frères. De plus, l'implication de *MYH11* dans le phénotype de dissection aortique [@Imai2015] l'ont écarté des candidats pour le phénotype MMAF.   
...
L'oligospermie, comme l'azoospermie sont des infertilités masculines liées à la quantité de spermatozoïdes présent dans l'éjaculât. Les différentes études publiées ces dernières années montrent que les microdélétions du chromosome Y sont retrouvées chez 10% des hommes avec une azoospermie non-obstructives et chez 5% des patients avec une
oligozoospermie sévères [@Hotaling2014]. Ces taux bien qu'élevé ne représente qu'une infime partie des cas d'azoospermie et d'oligospermie suggérant l'implication de nombreux autres gènes dans ce phénotype.  
...
. WES effectué sur les deux frères  

. tab des variants ayant passé les filtres

. fig de l'exp des gènes  

. The mutation was validated by Sanger sequencing in both brothers (homozygous) and their parents (heterozygous)

. PINK2 whole coding sequences of 611 patients affected by azoo- or oligozoospermia (210 patients with azoospermia, 393 subjects with oligozoospermia and 8 with unspecified cause Only one variant, identified in patient 105 (P105), was not described in ExAC and was likely deleterious (insert Table). This variant, c.1A>T (Fig EV1A), abrogates the SPINK2 start codon and was present heterozygously in P105, a man with oligozoospermia. An alternate start site could potentially be used in the middle of exon 3 allowing the synthesis of a truncated protein of 2 kDa lacking the reactive site and disulfide bonds, both known to be crucial for SPINK2 function  

. In order to confirm that the absence of SPINK2 leads to azoospermia, homozygous Spink2 KO ( / ) mice were obtained and their reproductive phenotype was studied 
. were completely infertile, whereas no reproductive defects were observed in females 




Moreover, despite normal fertility, heterozygous male mice had a high rate of morphologically abnormal spermatozoa and a reduced sperm motility
