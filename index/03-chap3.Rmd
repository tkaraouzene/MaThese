...
pair_stat <- pair_stat %>% 
  ...
  select(Run, Compatible_pairs, Non_compatible_pairs, `Orphans Any`) %>% 
  rename(`Compatible` = Compatible_pairs,
         `Non compatible` = Non_compatible_pairs,
         Orphans = `Orphans Any`) %>%
  gather(COMPATIBILITY,N,Compatible:Orphans) %>%
...
L'ensemble de nos exomes ayant été réalisés en *paired-end*, les deux extrémités de chaques fragments sont séquencés créant ainsi deux *reads*. Après avoir aligné indépendemment les deux *ends* d'un même *read*, seul ceux pour lesquels les deux *ends* présentent un alignement "compatible" sont conservés. Un alignement est dit "compatible" lorsque les deux *ends* s'alignent face à face (une sur le *strand* + et l'autre sur le *strand* - et couvrent une zone ne faisant pas plus de 3 fois la taille médiane de l'insert.

```{r plotrunstrandness}

ggplot(pair_stat, aes(COMPATIBILITY, N)) + 
  geom_boxplot(outlier.size = -1) + 
  facet_wrap(~COMPATIBILITY, scales = "free")
...
df_read_fate_nb <- fread("data/magic_results/All_Exomes.read_fate.txt", sep = "\t", data.table = FALSE, header = TRUE, drop = c(17:32)) %>% 
  ...
  rename(RUN = `# Run`,
         `1 site` = `Mapped uniquely to 1 genomic site and maximum 1 gene`,
         S2 = `Mapped at 2 to 9 sites`,
         S10 = `Rejected because they map to 10 or more sites`) %>% 
  filter(RUN != "All runs") %>%
  mutate(`2 or more sites`  = S2 + S10) %>% 
  select(-S2, -S10) %>% 
  filter(RUN %in% run_mmaf_fam) %>% 
  gather(WHAT,NB_OF_READS,`Raw reads`:`2 or more sites`) %>% 
  mutate(IS_CONSERVED = if_else(WHAT == "1 site", "Yes", "No"))
...
```{r plotreadsfate, fig.cap=fig.info,fig.scap=fig.name,fig.width=6, fig.height=6.5}

fig.name <- "Résultats du mapping des reads pour chaque Patients"
fig.description <- "**A** : Cette boite à moustache montre le nombre de *reads* bruts générés pour chaque patients (représentés par les points) au cours de l'étape de séquençage. On constate que ce nombre reste pour chaque patient dans le même ordre de grandeur sauf pour un des frères de la famille AZ  qui contient presque 35 millions de *reads* en plus que la mediane. **B** : Ce diagramme en barre montre le nombre de *reads* correctement allignés pour chaque patients. On peut donc constater que la quasi totalité de *reads* générés ont été correctement allignés pour l'ensemble des patients. **C** : Ces deux boites à moustaches montrent .... On constate que la grande majorité des *reads* ne mappent qu'à un seul site (boite bleue), les autres (boite rouge) seront d'ailleur écartés des analyses à suivre"
fig.info <- paste0(fig.name, " : ", fig.description)

pA <- ggplot(df_read_fate_nb %>% filter(WHAT == "Raw reads"), aes("", NB_OF_READS)) + 
geom_boxplot(outlier.size = -1) + 
...
pB <- ggplot(df_read_fate_nb %>% filter(WHAT == "Well mapped"), aes(RUN, NB_OF_READS)) + 
  geom_bar(stat = "identity") +
  ylab("Well mapped reads") 
  scale_x_discrete(labels = c("AZ1","AZ2","FF1","FF2","MMAF1.1","MMAF1.2","MMAF2.1","MMAF2.2","MMAF3.1","MMAF3.2","MMAF3.3"))

pC <- ggplot(df_read_fate_nb %>% filter(WHAT != "Well mapped" & WHAT != "Raw reads" & WHAT != "Unmapped" & WHAT != "Rejected by alignment quality filter"), aes(WHAT, NB_OF_READS)) + geom_boxplot(outlier.size = -1, aes(fill = IS_CONSERVED)) + 
  geom_jitter(size = 2, position=position_jitter(0.3)) +
  facet_wrap(~WHAT, scales = "free") + 
  ylab("Nb of reads") +
  xlab("") + 
  scale_fill_discrete(name = "Conserved reads?") +
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
  
row1 <- plot_grid(pA, labels = c("A"))
plot_grid(row1, pB, pC,labels = c("","B","C"), rel_heights = c(0.5,1,1), ncol = 1)
...
# TODO : mettre pour chaque un box plot montrant le nb de single strand, both_strand et filtered pour chaque patients 
# x <- rbind(df_fam_az,df_fam_ff,df_fam_mmaf1,df_fam_mmaf2,df_fam_mmaf3,df_fam_mmaf4)
# 
# x <- x %>% group_by(RUN, STRAND) %>% summarise(N = n_distinct(CHR,POSITION,REF,ALT))
# 
# ggplot(x, aes(STRAND,N)) + geom_boxplot()
...
fig.name <- "Comparaison du pouvoir discriminant de chaque filtre employé TODO mettre union"
fig.description <- "Les boites à moustache représentent la quantité de variants filtrés par chacun des 6 filtres utilisés sur l'ensemble des patients. Chaque point représente un patient et sa hauteur informe du nombre exact de variant filtrés par uin filtre donné. Comme on peut le voir, le filtre le plus discriminant est celui consistant à filtrer l'ensemble des variants hétérozygotes ce qui n'est pas surprenant puisqu'ils représentent entre ... et ... pourcents des variants observés pour chaque patients"
...
```{r largemmafcall, dependson="mmafdata", fig.cap=fig.info,fig.scap=fig.name, cache=TRUE}
...
```{r varperrun, dependson="mmafdata", cache=TRUE}
...
```{r dependson="mmafdata"}

n_gene_min <- nb_run_per_passing_gene$N_per_run %>% min
n_gene_max <- nb_run_per_passing_gene$N_per_run %>% max

n_gene_tot <- df_passing_filter_mmaf_genes %>% nrow

n_gene_1 <- filter(df_passing_filter_mmaf_genes, N_RUN == 1) %>% nrow
p_gene_1 <- ((n_gene_1 / n_gene_tot) * 100) %>% round(0)

n_gene_up_1 <- filter(df_passing_filter_mmaf_genes, N_RUN > 1) %>% nrow
p_gene_up_1 <- ((n_gene_up_1 / n_gene_tot) * 100) %>% round(0)

```

Les filtres utilisés ont été les mêmes que ceux détaillés dans l'études des cas familiaux, c'est à dire que seul les variants homozygotes ayant une fréquence $\le$ `r maf_cutoff` dans la population générales et n'étant pas observés dans la cohorte contrôle de `r nb_ctrl_mmaf` individus furent conservés. De même, les variants synonymes, impactant la séquences UTR ou chevauchant uniquement des transcrits annotés NMD par VEP on été filtrés. Ainsi, ces différents filtres nous ont permis d'obtenir une liste de `r nb_snv_tot` SNVs (entre `r nb_snv_min` et `r nb_snv_max` différents par patients) et de `r nb_indel_tot` indels (entre `r nb_indel_min` et `r nb_indel_max` différents par patients) (**Figure : **\@ref(fig:plotvarperrun)).  Cet ensemble de variant nous a alors permis d'obtenir des listes comprenant entre `r n_gene_min` et `r n_gene_max` gènes différents par patients (**Figure : **\@ref(fig:plotpassinggenes) - **A**) constituant ainsi un total de `r n_gene_tot` gènes différents pour l'ensemble des patients. Parmi ceux là, `r n_gene_1` (`r p_gene_1`%) ont été retrouvés mutés chez un seul patients tandis que `r n_gene_up_1` (`r p_gene_up_1`%) ont été retrouvé chez au moins 2 patients (**Figure : **\@ref(fig:plotpassinggenes) - **B**). 

```{r plotvarperrun, dependson="mmafdata", fig.cap=fig.info,fig.scap=fig.name, echo=FALSE, cache=TRUE}
...
```{r plotpassinggenes, dependson="mmafdata", fig.cap=fig.info,fig.scap=fig.name, fig.height=6.5, fig.width=6}
...
```{r cil, dependson="mmafdata"}

n_gene_cil <- df_ciliome_gene %>% select(SYMBOL) %>% distinct() %>% nrow


df_ciliome <- df_filter_mmaf_all %>% 
  group_by(EVIDENCE) %>% 
  filter(!is.na(EVIDENCE)) %>% 
  group_by(SYMBOL) %>%
  mutate(N_RUN = n_distinct(RUN)) %>% 
  distinct(EVIDENCE, N_RUN) %>% 
  arrange(desc(N_RUN)) %>%
  ungroup() %>%
  select(SYMBOL, N_RUN, EVIDENCE)
  
```


Le flagelle spermatique pouvant s'apparenté à un cil, nous avons ensuite comparés ces gènes avec une liste de `r n_gene_cil` gènes prédits comme faisant parti du ciliome [TODO: insert ref]. Ainsi, sur notre ensemble de gène ayant passé les filtres `r df_ciliome %>% nrow` sont prédit comme faisant parti du ciliome (**Table :** \@ref(tab:tabciliome)). 

```{r tabciliome, dependson="mmafdata"}

tab.name <- "Gènes ayant passé les filtres et annotés comme faisant partie du ciliome"

kable(df_ciliome, 
      col.names = c("Gene", "Patient carrying a variant on the gene"," Ciliome evidence"),
      caption = tab.name, 
      longtable = TRUE, 
      booktabs = TRUE)
...
# Mettre table des candidats évidents
...
# À mettre en annexe
# ##### Filtrer de sorte à obtenir la liste des candidats évidents (demander liste à Pierre)
# tab.name <- "Liste des gènes passant les filtres chez au moins deux patients !!!!! TODO CECI est un header, à changer !!!!!"
# kable(head(df_passing_filter_mmaf_genes %>% filter(N_RUN >= 2), n = 30), caption = tab.name, col.names = c("Gene", "Nb of individuals"), longtable = TRUE, booktabs = TRUE) 
...
```{r filterhetmmaf, cache=TRUE}

# <!--     b. Liste complète des gènes avec 2 variants tronquants -->

df_filter_het_mmaf <- df_mmaf_all %>%
  filter_genotype(homo = FALSE) %>%
  variant_join(df_ctrl_mmaf, mode = "anti") %>%
  variant_join(df_all_coding_variants, mode = "inner") %>%
  filter(!grepl("NMD_transcript_variant",Consequence),
         IMPACT == "HIGH") %>% 
  filter_frequence(cutoff = maf_cutoff) %>%
  select(-CADD_RAW, -IMPACT,-(STRAND:`1KG`), -(SIFT:ExAC_AF)) %>%
  group_by(RUN, Feature, SYMBOL) %>%
  mutate(N_PER_RUN = n()) %>% 
  filter(N_PER_RUN > 1) %>%
  group_by(RUN, N_PER_RUN) %>%
  distinct(SYMBOL) %>% 
  ungroup() %>% 
  mutate(N_TOT_GENE = n_distinct(SYMBOL))
  
```

```{r plotfilterhetmmaf, cache=TRUE}
# 
# fig.name <- "Analyse des gènes passant les filtres"
# fig.description <- "**A**: Représentation du nombre de gène ayant passé l'ensemble des filtres pour chaque individus (chaque point correspond à un individu), **B**: Visualisation du nombre de patient portant au moins un sur le même gène"
# 
# ggplot(df_filter_het_mmaf, aes("",)) +
#   geom_boxplot(width = .5, stat = "count") + 
#   geom_jitter(position=position_jitter(0.2)) + 
#   ylab("Nb of passing filter gene") + 
#   theme(axis.title.x = element_blank(),
#         axis.text.x = element_blank(),
#         axis.ticks.x = element_blank())
# 
# ggplot(df_filter_het_mmaf, aes(N_PER_RUN)) +
#   geom_text(aes(label = ..count..),  position = position_dodge(width = .9), vjust = -.25, stat= "count") +
#   geom_bar(position = position_dodge(),fill = ) +
#   ylab("Nb of Gene") +
#   xlab("Nb of patient carrying a variant on the same gene") +
...
# plot_grid(pA,pB, ncol = 1, rel_heights = c(1, 1.8), labels = c("A","B"))
# 
# ggplot(df_filter_het_mmaf, aes(RUN)) + 
# geom_bar()
# # 
# 
# df_filter_het_mmaf %>% group_by(SYMBOL) %>% mutate(N_RUN = n()) %>% ungroup() %>% mutate(N_GENE_TOT = n_distinct(SYMBOL)) %>% ungroup() %>% distinct(SYMBOL) %>% View
# 
