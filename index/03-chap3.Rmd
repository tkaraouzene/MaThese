...
gene_grp3_retained <- c("CCDC65", "C6orf118","C21orf59")

df_grp3_high_reduce <- df_grp3_high %>%
  filter(SYMBOL %in% gene_grp3_retained)

```

```{r vargrp3reduce}

n_gene_grp3_retained <- gene_grp3_retained %>% length
run_grp3_retained <- df_grp3_high_reduce %>% distinct(RUN) %>% collect() %>% .[["RUN"]]
n_run_grp3_retained <- run_grp3_retained %>% length
...
run_found <- c(run_found, run_grp3_retained)
...
1. **Étape n°1** : La recherche de variants homozygote tronquant nous a permis d'identifier `r n_run_grp3_high` patients d'un variant sur un des `r n_gene_grp3` gène suivant : `r gene_grp3`.  Cependant, l'analyse de l'expression testiculaire de ces gènes nous a insitée à n'en garder que `r gene_grp3_retained` d'entre eux : `r gene_grp3_retained` (**Table :** \@ref(tab:tabgrp3high), **Figure :** \@ref(fig:plotgrp3) - **A**). . En revanche .  
