...
kable(df_runs_mmaf %>% 
        ...
        arrange(Year)
      ,booktabs = TRUE, caption = "Liste des différents projets de séquençages effectués")
...
# 
# 
# df_bigmmaf_call_stat <- df_bigmmaf_call %>%
#   group_by(RUN, CALL, STRAND, TYPE) %>%
#   distinct(CHR, POSITION, REF, ALT) %>%
#   mutate(N_PER_STRAND = n()) %>% 
#     group_by(RUN, CALL, TYPE) %>% 
#   mutate(N_BOTH_STRAND = sum(N_PER_STRAND),
#          P_STRAND = N_PER_STRAND / N_BOTH_STRAND) %>%
#   left_join(df_runs_mmaf, by = "RUN") %>% 
#   select(-(Project:Model))
...
pC <- ggplot(df_bigmmaf_call_strand, aes(reorder(PROJECT, YEAR), P_STRAND)) + 
  ...
  facet_wrap(~factor(STRAND, levels = c("DS", "SS", "NS")), scales = "free") +
  scale_x_discrete(limits = c("Strasbourg (2012)", "MountSinai (2012)", "Genoscope (2013)", "Genoscope (2014)", "Genoscope (2015)")) +
  scale_fill_discrete(name = "Sequencing project", breaks = c("Strasbourg (2012)", "MountSinai (2012)", "Genoscope (2013)", "Genoscope (2014)", "Genoscope (2015)")) +
  ylab("% of call") + 
  scale_y_continuous(labels = percent) +
  theme(legend.position = "bottom",
...
df_distinct_call <-  df_distinct_call %>%
...
tabdnah1 <- df_bigmmaf_filter %>% 
  filter(SYMBOL == "DNAH1") %>% 
  mutate(VARIANT = paste(CHR,POSITION,REF,ALT, sep = "-"),
         Consequence = sapply(strsplit(Consequence, "_", fixed = TRUE),"[[",1)) %>%
  distinct(RUN, VARIANT, Consequence, SYMBOL)

n_run_dnah1 <- tabdnah1 %>% ungroup() %>% distinct(RUN) %>% nrow
n_run_dnah1 <- tabdnah1 %>% ungroup() %>% distinct(RUN) %>% nrow
...
kable(tabdnah1, 
      booktabs = TRUE,
      longtable = TRUE,
      col.names = c("Run ID","Variant coordinates", "Consequence", "Gene symbol"),
...
```{r plotfilterbigmmaf, fig.cap=fig_info,fig.scap=fig_name, fig.height=6, cache=TRUE}
