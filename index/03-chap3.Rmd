...
var_call <- function(file, fam, run) {
...
Pour les positions SS, l'appel final sera directement .... Pour les positions DS, la concordance des appels fournis par chaque *end* est verifié. Ainsi, un variant sera considéré :
...
### La priorisation des gènes  

Dans le cas d'analyses de larges cohortes de patients, il arrive souvent que de nombreux variants aient passé l'ensemble des filtres ceux-ci pouvant affecter plusieurs centaines de gènes. Il es dès lors nécéssaire d'appliquer une méthode de priorisation permettant de classer les gènes en fonction de leur probabilité à être responsables du phénotype étudié. Pour celà, nous avons mis en place .......


\newpage

## Résultats 1 : Analyse de 3 cas familiaux  
...
run_all_fam <- c(run_fam_az, run_fam_ff, run_fam_mmaf3)
...
#   rename(ID = `Magic ID`) %>% 
#   collect() %>%
#   .[["ID"]] %>% paste_vector(sep = "\", \"",  last = "\", \"")
# 


df_fam_run <- data_frame(FAM = c("AZ","AZ","FF","FF","MMAF3","MMAF3"),
                     RUN = c(run_fam_az, run_fam_ff, run_fam_mmaf3),
                     PHENOTYPE = c(rep("Azoospermia", 2), rep("Fertilization failure", 2),  rep("MMAF", 2)),
                     YEAR = c(2012, 2012, 2014, 2014,2014, 2014),
                     # PLATFORM = c("Illumina HiSeq2000", "Illumina HiSeq2000", "Illumina HiSeq2000"),
                     PLACE = c(rep("Mount Sinai Institut", 2), rep("Genoscope (Evry)", 4)))
...
```{r tabfam, cache=TRUE, dependson="dffam"}
...
```{r dffammapping, warning=FALSE, cache=TRUE}
...
if (!file.exists(call_file)) {
  ...
  df_call_fam <- bind_rows(
    var_call(file = "data/call_results/call_az1.csv", fam = "AZ", run = "AZ1"),
    var_call(file = "data/call_results/call_az2.csv", fam = "AZ", run = "AZ2"),
    var_call(file = "data/call_results/call_ff1.csv", fam = "FF", run = "FF1"),
    var_call(file = "data/call_results/call_ff2.csv", fam = "FF", run = "FF2"),
    var_call(file = "data/call_results/call_mmaf2_1.csv", fam = "MMAF", run = "MMAF1"),
    var_call(file = "data/call_results/call_mmaf2_2.csv", fam = "MMAF", run = "MMAF2")
    )
...
df_run_ovo <- fread("data/exome_runs.csv", sep = "\t", header = TRUE, showProgress = FALSE, verbose = FALSE, drop = c(2,3,4,10:16), data.table = FALSE) %>% 
  dplyr::rename(RUN = `Magic ID`, PHENOTYPE = Pathologie, DATE = Date) %>%
...
```{r dfovocall, cache=TRUE}

ovo_call_file <- "results/ovo_call.rds"
ovo_distinct_call_file <- "results/ovo_distinct_call.rds"

if (!file.exists(ovo_call_file)) {

  df_ovo_call <- fread("data/call_results/genotype_ovo.csv", sep = "\t", header = TRUE, showProgress = FALSE, verbose = FALSE) %>% 
    gather(RUN, CALL, starts_with("Ghs")) %>% 
  ...
  saveRDS(df_ovo_call, ovo_call_file)

} else {

  df_ovo_call <- readRDS(ovo_call_file)

}

if (!file.exists(ovo_distinct_call_file)) {

df_distinct_ovo_call <- df_ovo_call %>%
  filter(grepl("zygous", CALL)) %>%
  distinct(CHR, POSITION, REF, ALT, TYPE) %>%
  mutate(CHR = as.character(CHR)) %>%
  variant_join(fread("~/all_chr_var.csv",  data.table = FALSE, sep = "\t", header = TRUE, showProgress = FALSE, verbose = FALSE), mode = "left") %>%
         mutate(PolyPhen = gsub("_", " ", str_extract(PolyPhen, "\\w+")),
              SIFT = gsub("_", " ", str_extract(SIFT, "\\w+")),
              Consequence = sapply(strsplit(Consequence, "_variant", fixed = TRUE),"[[",1),
              Consequence = gsub("_", " ", Consequence),
  ...
  saveRDS(df_distinct_ovo_call, ovo_distinct_call_file)

} else {

  df_distinct_ovo_call <- readRDS(ovo_distinct_call_file)
...
# 
# nb_ovo_distinct_var <- df_distinct_ovo_call %>% nrow %>% as.integer
# nb_ovo_distinct_snv <- df_distinct_ovo_call %>% filter(TYPE == "SNV") %>% nrow %>% as.integer
# nb_ovo_distinct_indel <- df_distinct_ovo_call %>% filter(TYPE == "Indel") %>% nrow %>% as.integer
# 
# nb_ovo_mean_var <- df_ovo_call_geno %>% group_by(RUN) %>%  summarise(N = n_distinct(CHR, POSITION, REF ,ALT)) %>% collect() %>% .[["N"]] %>% mean %>% round(0) %>% as.integer
# nb_ovo_mean_snv <- df_ovo_call_geno %>% filter(TYPE == "SNV") %>% group_by(RUN) %>% summarise(N = n_distinct(CHR, POSITION, REF ,ALT)) %>% collect() %>% .[["N"]] %>% mean %>% round(0) %>% as.integer
# nb_ovo_mean_indel <- df_ovo_call_geno %>% filter(TYPE == "Indel") %>% group_by(RUN) %>% summarise(N = n_distinct(CHR, POSITION, REF ,ALT)) %>% collect() %>% .[["N"]] %>% mean %>% round(0) %>% as.integer
# 
# nb_ovo_mean_homo <- df_ovo_call_geno %>% filter(CALL == "Homozygous") %>% group_by(RUN) %>% summarise(N = n_distinct(CHR, POSITION, REF ,ALT)) %>% collect() %>% .[["N"]] %>% mean %>% round(0) %>% as.integer
# p_ovo_mean_homo <- (nb_ovo_mean_homo / nb_ovo_mean_var)  %>% round(3) * 100 
# 
# # ns_ovo_geno_2015_mean <- df_ovo_call_strand %>% filter(PROJECT == "Genoscope (2015)", STRAND == "NS") %>% collect() %>% .[["P_STRAND"]] %>% median %>% round(3) * 100

```

<!-- # FILTER -->

```{r dfovofilter, cache=TRUE, dependson="dfovocall"}

df_ctrl_ovo <- open_ctrl("data/call_results/genotype_not_ovo_not_azoo.csv", pheno = "Ovo")

maf_cutoff <- .01

df_distinct_ovo_filter <- df_distinct_ovo_call %>%
  filter_frequence(cutoff = maf_cutoff) %>%
  variant_join(df_ctrl_ovo %>% mutate(CHR = as.character(CHR)), mode = "anti") %>%
  filter(Consequence != "synonymous",
         IMPACT != "MODIFIER",
         !(grepl("tolerated", SIFT) & grepl("benign", PolyPhen)),
...
Entre `r min_ovo_year` et `r max_ovo_year` notre équipe a pris en charge le diagnostique de `r nb_ovo_run` femmes nord africaine présentant toutes une déficience méiotique ovocytaire (DMO) caractérisé par un blocquage de la méïose au stade M1 et unduisant une infertilité. Un séquençage exomique a été fait sur ces `r nb_ovo_run` femmes. 


Dans ce contexte, j'ai, au cours de ma thèse, été en charge de l'ensemble des analyses bio-informatiques ayant permi l'identification des variants de ... femmes ayant été séquencées en haut débit.  
...
run_bigmmaf_novogene <- c("15IF060", "15IF059", "16IF032", "10IF017", "15IF063", "15IF061", "15IF065", "16IF030", "16IF036", "15IF064")
run_ctrl_bigmmaf_novogene <- c("s15IF014", "s15IF018", "s15IF013", "s15IF016", "s15IF012", "s15IF015", "s15IF019", "s15IF017", "s15IF011")



run_all_fam <- c(run_az_fam, run_ff_fam, run_mmaf_fam)

df_run_bigmmaf <- fread("data/exome_runs.csv", sep = "\t", header = TRUE, showProgress = FALSE, verbose = FALSE, drop = c(2,3,4,10:16), data.table = FALSE) %>%
...
nb_run_bigmmaf <- df_run_bigmmaf %>% nrow

```

<!-- CALL -->

```{r dfbigmmafcall, cache=TRUE}

bigmmaf_call_file <- "results/bigmmaf_call.rds"
bigmmaf_distinct_call_file <- "results/bigmmaf_distinct_call.rds"
df_anotate_file <- "data/call_results/annotation_old_and_novogene_variant.rds"

if (!file.exists(bigmmaf_call_file)) {
  
  df_novogene_bigmmaf_indel_call <- fread("data/call_results/novogene_patients_indel_genotype.csv", 
                                          sep = "\t", header = TRUE, showProgress = FALSE, verbose = FALSE, data.table = FALSE) %>% 
    gather(RUN, CALL, starts_with("1")) %>%
    filter(CALL %in% c(0, 2, 3, "b", "c"),
           !(RUN %in% run_bigmmaf_novogene),
           !is.na(REF)) %>%
    mutate(STRAND = call_to_strand(CALL),
           TYPE = var_type(RES, ALT),
           CALL = call_to_geno(CALL))
  
  df_novogene_bigmmaf_snp_call <- fread("data/call_results/novogene_patients_snp_genotype.csv", sep = "\t", header = TRUE, showProgress = FALSE, verbose = FALSE, data.table = FALSE) %>% 
    gather(RUN, CALL, starts_with("1")) %>%
    filter(CALL %in% c(0, 2, 3, "b", "c"),
           !(RUN %in% run_bigmmaf_novogene),
           !is.na(REF)) %>%
  ...
  df_bigmmaf_call <- fread("data/genotype_MMAF.csv", sep = "\t", header = TRUE, showProgress = FALSE, verbose = FALSE) %>% 
    gather(RUN, CALL, starts_with("Ghs")) %>%
    filter(CALL %in% c(0, 2, 3, "b", "c"),
           !(RUN %in% c(run_mmaf_fam, run_bigmmaf_novogene)),
           !is.na(REF)) %>%
    mutate(CHR = as.character(CHR),
           STRAND = call_to_strand(CALL),
           TYPE = var_type(RES, ALT),
           CALL = call_to_geno(CALL)) %>%
    bind_rows(df_novogene_bigmmaf_indel_call) %>%
    bind_rows(df_novogene_bigmmaf_snp_call)
...
if (!file.exists(bigmmaf_distinct_call_file)) {
  ...
  if (!file.exists(df_anotate_file)) {
    
    # retrieve the VEP annotation for each variants
    df_anotate <- fread("~/all_chr_var.csv", sep = "\t", header = TRUE, showProgress = FALSE, verbose = FALSE) %>% 
      bind_rows(fread("data/call_results/novogene_patients_distinct_var_vep_format.csv", sep = "\t", header = TRUE, showProgress = FALSE, verbose = FALSE)) %>%
      arrange(CHR, POSITION, REF, ALT) %>%
      distinct()  
    
    saveRDS(df_anotate, df_anotate_file)
    
  } else {
    
    df_anotate <- readRDS(df_anotate_file)
  ...
  df_distinct_call <- df_bigmmaf_call %>%  
    ...
    variant_join(df_anotate, mode = "left") %>%
    mutate(PolyPhen = gsub("_", " ", str_extract(PolyPhen, "\\w+")),
           SIFT = gsub("_", " ", str_extract(SIFT, "\\w+")),
           Consequence = sapply(strsplit(Consequence, "_variant", fixed = TRUE),"[[",1),
           Consequence = gsub("_", " ", Consequence),
...
```{r dfctrlbigmmaf, cache=TRUE,  dependson= c("dfbigmmafcall", "localfunction")}

df_novogene_bigmmaf_indel_ctrl <- fread("data/call_results/novogene_patients_indel_genotype.csv", sep = "\t", header = TRUE, showProgress = FALSE, verbose = FALSE, data.table = FALSE) %>% 
  select(CHR:ALT,`15IF014`, `15IF018`, `15IF013`, `15IF016`, `15IF012`, `15IF015`, `15IF019`, `15IF017`, `15IF011`) %>% 
  open_ctrl(df = ., pheno = "MMAF")

df_ctrl_mmaf <- df_novogene_bigmmaf_indel_ctrl %>% 
  bind_rows(open_ctrl("data/genotype_not_MMAF.csv", pheno = "MMAF")) %>%
  select(-N_CTRL, -PHENO) %>%
  distinct(CHR, POSITION, REF, ALT) 

rm(df_novogene_bigmmaf_indel_ctrl)
gc()

```

```{r dfbigmmaffilter, dependson= c("dfbigmmafcall", "extradate"), cache=TRUE}
...
# 
tab_cfap43 <- fread("/home/karaout/MaThese/index/data/tab_cfap43.csv", sep = "\t", data.table = FALSE, header = TRUE, showProgress = FALSE, verbose = FALSE) %>%
  pander()
#   # mutate(`HGVSc ; HGVSp` = paste0(HGVSc, " ; ", HGVSp)) %>% 
#   # select(-HGVSc, -HGVSp) %>% 
#   # arrange(desc(Genotype), `HGVSc ; HGVSp`) %>%
#   as.data.frame()
# 
# kable(tab_cfap43,
#       format = "latex",
...
L'application de de notre pipeline d'analyse sur les données de ces ... patients nous a permi d'obtenir une liste de ... variant distincts avec une moyenne de ... par patients (insert fig) sur lesquels nous avons appliqués les filtres précédemments décrits soit : 

répéter les filtres utilisés). 

L'application de ces filtres nous a ainsi permis d'obtenir une liste "restreinte" de ... de SNV et ... indels impactant ... gènes différents (liste en annexe). Au vu du nombre important de gène restant et afin d'étudier en priorité ceux pouvant expliquer le phénotype d'un maximum de patients nous avons, comme précédemment, limité nos recherches aux gènes sur lesquels **au moins 2 patients portaient un variant tronquant** réduisant à nouveaux cette liste à ... gènes différents (**Figure : **\@ref(fig:plottwohomohigh)).

En raison du nombre important de patients portant des mutations tronquantes sur les gènes *CFAP43* et *CFAP44* (**Table : **\@ref(tab:tabcfap43)), couplé au fait que les protéines codées par ces deux gènes présentent toutes deux des motif WDR, ces gènes furent les premiers que nous avons cherché à caractériser.  

\newpage

```{r plottwohomohigh, cache=TRUE, dependson="dftwohomohigh"}
...
# 
# df_cfap <- bind_rows(df_bigmmaf_filter, df_bigmmaf_filter_het) %>%
#   filter(CANONICAL == "YES") %>%
...
# 
#    kable(df_cfap %>% filter(Gene == "DNAH1") %>%  select(-Gene) %>% arrange(desc(Geno), Patient, `HGVSc, HGVSp`),
#      format = "latex",
#           booktabs = TRUE,
#           longtable = TRUE,
#           caption = caption) %>%
...
#   select(Patient = RUN, Gene = SYMBOL, Geno = CALL, `HGVSc, HGVSp`, Consequence, `ExAC ; ESP ; 1KG`) 
# 
#    kable(df_cfap %>% filter(Gene == "CFAP44") %>%  select(-Gene) %>% arrange(desc(Geno), Patient, `HGVSc, HGVSp`),
#      format = "latex",
#           booktabs = TRUE,
#           longtable = TRUE,
#           caption = caption) %>%
