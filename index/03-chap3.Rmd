...
tab_high <- function(df) {
  
  df %>% ungroup() %>% 
    select(Patient = RUN, Gene = SYMBOL, Evidence = EVIDENCE, Consequence, ESP, `1KG`, ExAC = ExAC_AF) %>%
    distinct() %>% 
    arrange(Gene) %>% 
    mutate(ESP = na.replace(ESP, "none"),
           `1KG` = na.replace(`1KG`, "none"),
           ExAC = na.replace(ExAC, "none"))
}


tab_modelow <- function(df) {
  
  df %>% ungroup() %>%
    select(Patient = RUN, Gene = SYMBOL, Consequence, SIFT, PolyPhen, ESP, `1KG`, ExAC = ExAC_AF) %>%
    distinct() %>% 
    mutate(ESP = na.replace(ESP, "none"),
           `1KG` = na.replace(`1KG`, "none"),
           ExAC = na.replace(ExAC, "none"),
           SIFT = na.replace(SIFT, "none"),
           PolyPhen = na.replace(PolyPhen, "none"))
}
...
df_bigmmaf_genes_resume <- df_bigmmaf_genes_distinct %>% 
  ...
  summarise(N_GENE = n()) %>%
  mutate(MORE_1_RUN = if_else(N_RUN == 1, "One signle run", "Several runs"))
...
2. **Étape n°2** : Pour l'ensemble des gènes retenus dans l'étape n°1, nous recherchons ensuite des patients portant, **toujours à l'état homozygote**, des variant aux effets **non tronquants** tel que des variants faux-sens ou encore des variants intronique situés proches des sites d'épissage. Dans le cas des variants faux-sens, les logiciels SIFT et PolyPhen sont ensuite utilisés afin de nous orienter quant à l'effet délétère du variant, bien que comme nous l'avons déjà vu, ces logiciels son contredisent regulièrement [TODO : ref!!!]. Au vus de la difficulté à déterminer l'effet délétère de ces variants, les preuves génétiques seront considéré comme de confience modérée (*Moderate trust*).    

3. **Étape n°3** : Cette étape consiste à recherchere des patients éventuellement hétérozygotes composites, c'est à dire des patients portant deux variants hétérozygotes différents sur chacun des deux allèles d'un même gène. Malheuresement, dans le cadre des séquençages WES et WGS, il est impossible de connaitre le "phasage" des variants, c'est à dire que l'on ne peut déterminer si deux variants hétérozygotes sont situés sur le même allèles ou sur deux allèles différents. Pour cela, des analyses de biologie moléculaire sont nécéssaire. C'est pour cette raison que ces preuves génétiques seront considérées comme étant de faible confience (*Low trust*). 


```{r}

# TODO mettre schéma hétérozygote composites  
...
```{r removednah1, dependson=c("extradata","filterbigmmaf"), cache=TRUE}
...
1. **Étape n°1** : Parmis l'ensemble de nos `r n_run_bigmmaf` patients MMAF `r n_run_dnah1_high` portait un indel homozygote entrainant un décalage du cadre de lecture sur le gène *DNAH1* (**Table : **\@ref(tab:tabgrp1high).  
2. **Étape n°2** : La recherche de variants homozygote non tronquant sur le gène *DNAH1* nous a permis d'identifier `r n_run_dnah1_moderate` nouveaux patients : ... . On note ainsi que le patient *Ghs90* est porteur de ... variants homozygotes successif, tous entrainant des faux-sens prédits comm *benign* par PolyPhen et pour lesquels SIFT ne proposen aucune prédiction. Le patient *Ghs95*, lui porte égallement un variant faux-sens différent de ... observés chez *Ghs90* mais également prédit comme *benign* par PolyPhen. Il est cependant à ce stade impossible de conclure avec certitudes que ces variants soient effectivement responsables du phénotype MMAF de ces `r n_run_dnah1_moderate` patients compte tenus des prédictions fournis par PolyPhen et du fait qu'il est difficile de conclure quant à l'effet d'un variant-faux-sens sans effectuer d'analyses fonctionelles. Cependant, l'abscence de ces ... dans chacune des 3 bases de donées que nous avons utilisés, c'est à dire ExAC, 1KG et ESP6500, laisse supposer que ceux-ci soient êtremement rare 
3. **Étape n°3** : La recherche d'hétérozygote composites nous a permis de révéler `r n_run_dnah1_low` patients portant tous 2 variants hétérozygotessur le gène *DNAH1*. On oeut alors noter le patients *Ghs36* portant 2 variant hétérozygotes. Le premier d'entre eux créé un codon stop de manière prématuré et n'est retrouvé ni dans ESP6500 ni dan 1KG tandis que sa fréquence dans la base de donnée ExAC est de .... Le second est un variant faux-sens abscent des 3 bases de données et prédit comme *probably damaging* par PolyPhen. Ainsi, s'ils étaient situés sur deux allèles différents, ces deux variants pourraient être des bons candidat pour expliquer le phénotype du patient *Ghs36*. On peut noter égallement le patient *Ghs129* portant deux faux-sens hétérozygote tout deux prédits comme *probably damaging* par PolyPhen et dont un seul est retrouvé dans la base de donnée exac avec une fréquence de ... . Pour les ... autres patients, il est plus dificile de se prononcer à ce stade compte tenus des fréquence parfois élevée des variants ou bien des prédictions fournies par PolyPhen.  
4. **Étape n°4** : Au vus des résultats, seuls les données du patient *`r run_dnah1_high`* ont été retirées de notre liste de variants, celui-ci étant le seul à porter un variant homozygote tronquant. Cela a ainsi permis de réduire notre liste à `r n_distinct_var2` variants distincts cheveauchant `r n_distinct_gene2` gènes différents.  

Ainsi, cette première analyse nous a permis de révéler que `r n_run_dnah1_high + n_run_dnah1_moderate + n_run_dnah1_low` des `r n_run_bigmmaf` patients de notre cohorte portaient au moins 1 variant sur le gène *DNAH1* et que pour `r n_run_dnah1_high + n_run_dnah1_moderate` d'entre eux ce(s) variants étaient présents à l'état homozygote. Cependant, il faut noter que du fait de son effet tronquant sur la proétine, seul le variant homozygote porté par le patient *Ghs122* nous permets d'être certains de la causalité du phénotype MMAF. Pour les `r n_run_dnah1_moderate + n_run_dnah1_low` autres patient, des analyses fonctionelles complémentaires sont nécéssaires (**Figure : **\@ref(fig:plotdnah1) - **B**).  
...
kable(tab_high(df_dnah1_high),
      booktabs = TRUE,
      longtable = TRUE,
      caption = "Liste des patients portant au moins un variant homozygote tronquant sur le gène *DNAH1*")
...
kable(tab_modelow(df_dnah1_moderate),
      booktabs = TRUE,
      longtable = TRUE,
      caption = "Liste des patients portant au moins un variant homozygote non tronquant sur le gène *DNAH1*")

```

```{r}

kable(tab_modelow(df_dnah1_low),
      booktabs = TRUE,
      longtable = TRUE,
      caption = "Liste des patients portant au moins deux variant hétérozygotes sur le gène *DNAH1*")
...
```{r tabgrp1high, dependson=c("localfunction","dfgrp1high"), cache=TRUE}

kable(tab_high(df_grp1_high),
      booktabs = TRUE,
      longtable = TRUE,
      caption = "List des gènes présents dans la liste ciliome sur lesquels au moins deux patients portent une mutation tronquante homozygote")

```

```{r tabgrp1moderate, dependson="localfunction", cache=TRUE}

gene1 <- paste_vector(gene_grp1_high)
cap <- paste0("Liste des patients portant un variant non troquant homozygote sur un des gènes suivant : ", gene1)

kable(tab_modelow(df_grp1_moderate),
      booktabs = TRUE,
      longtable = TRUE,
      caption = cap)
...
```{r removegrp2, cache=TRUE, dependson="dfgrp2retained"}
...
kable(tab_high(df_grp2_high) %>% select(-Evidence),
      booktabs = TRUE,
      longtable = TRUE,
...
kable(tab_modelow(df_grp2_moderate),
      booktabs = TRUE,
      longtable = TRUE,
...
fig_name <- "Analyse des gènes séléctionnés dans l'Analyse n°3"
...
```{r tabgrp3high, dependson="dfgrp3high", cache=TRUE}

kable(tab_high(df_grp3_high),
      booktabs = TRUE,
      longtable = TRUE,
      caption = "Analyse n°3 : List des gènes présents dans la liste ciliome sur lesquels un seul patient portent une mutation homozygote tronquante")

```

```{r tabgrp3low, dependson="dfgrp3low", cache=TRUE}

kable(tab_modelow(df_grp3_low),
      booktabs = TRUE,
      longtable = TRUE,
      caption = paste0("Analyse n°3 : Liste des patients portant au moins deux variants hétérozygotes sur un des gènes suivant : ",paste_vector(gene_grp3,"itallic")))
...
1. *CCDC66* : https://www.ncbi.nlm.nih.gov/pubmed/28235840

```{r, fig.height = 7.5}

retrieve_exp(gene_grp4) %>% filter(Tissues == "testis") %>% 
  ggplot(aes(reorder(SYMBOL, P_Expression), P_Expression)) +
    geom_col(aes(fill = P_Expression)) + 
  viridis::scale_fill_viridis(labels = percent) + 
  coord_flip()
...
2. **Étape n°2** : Nous avons donc cherché des patients portant au moins 1 variant homozygote non tronquant sur l'un de ces `r n_gene_grp4`. Nous avons ainsi pu identifier `r n_run_grp4_moderate` 

\newpage

```{r, fig.height=7.5}
# plot_expression(gene_grp4_moderate_reduce,"percent")
...
```{r tabgrp4moderate, dependson="bigmmafgrp1", cache=TRUE}

kable(df_grp4_moderate %>% 
        arrange(SYMBOL) %>%
...
df_resume_bigmmaf <- bind_rows(df_resume_dnah1 %>% mutate(ANALYSE = "Analysis DNAH1"),
                               ...
                               df_resume_grp2 %>% mutate(ANALYSE = "Analysis n°2"),
...
# 
# 
# pour les analyses familiales : 
#   
#   résultat plus complets avec ref c. et p.


# 
# bind_rows(df_dnah1_high,
#           df_dnah1_moderate,
#           df_dnah1_low,
#           
#           df_grp1_high,
