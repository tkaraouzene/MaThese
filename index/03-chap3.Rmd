...
#   
#   df_ovo_call <- fread("data/call_results/call_ovo.csv", sep = "\t", header = TRUE, showProgress = FALSE, verbose = FALSE) %>% 
#     gather(RUN, CALL, starts_with("Ghs")) %>%
...
# } else {
#   
#   df_distinct_call <- readRDS(ovo_distinct_call_file)
#   
# }
...
Après avoir appliqué les mêmes filtres que ceux décrit précédemment à l'exception du filtre n°1 ["Union"](#filterdescription) puisqu'ici nous avons uniquement des individus non apparentés, nous avons pu obtenir une liste de `r n_distinct_var` variants différents composés de `r n_distinct_snv` SNVs et `r n_distinct_indel` indels et impactant un total de `r n_distinct_gene` gènes distincts. Ces variants étant répartis sur l'ensemble de nos `r nb_run_bigmmaf` patients ceux-ci portaient en moyenne `r n_mean_snv` SNVS et `r n_mean_indel` indels, de sorte que chacun d'entre eux avaient entre `r n_min_gene` et `r n_max_gene` gènes impactés par au moins un variants homozygote (**Figure : **\@ref(fig:plotfilterbigmmaf) - **A** et **B**).  

```{r plotfilterbigmmaf, dependson="dfbigmmaffilter", fig.cap=fig_info,fig.scap=fig_name, fig.height=5, cache=TRUE}
...
Au vu du grand nombre de gène, et afin d'étudier en priorité ceux pouvant expliquer le phénotype d'un maximum de patients nous avons, dans un premier temps, limité nos recherches aux gènes sur lesquels **au moins 2 patients portaient un variant tronquant** . Ainsi nous avons obtenu une liste restreinte de ... gènes différents. Ensuite, nous avons chercher l'ensemble des patients portant des variants homozygotes **non tronquant** sur au moins un de ces gènes.  

3. récupérer les patients ayant au moins deux hétérozygotes  
...
ggplot(df, aes(reorder(reorder(reorder(SYMBOL,N_LOW), N_MODERATE), N_HIGH), N_RUN)) + 
  geom_col(aes(fill = EVIDENCE)) +
  scale_fill_manual(name = "Evidence", values = c("#4DAF4A", "#FF7F00", "#E41A1C")) + 
  scale_y_continuous(breaks = (1:max(df$N_TOT))) +
  ylab("Nb of individuals") + 
  xlab("") +
  coord_flip() 
...
Après avoir identifié le gène *DNAH1* comme un acteur majeur du syndrome MMAF, notre équipe s'est en partie spécialisée dans la caractérisation de ce phénotype. C'est pourquoi, entre ... et ... nous avons effectué le séquençage WES de ... individus. Parmi ceux-ci, ... ont pu être analysés au sein de notre pipeline d'analyse tandis que pour des raisons materiels, les ... autres n'ont pu l'être. Pour ces derniers nous avons donc utilisé directement les variants fournis par les centres en charge du séquençage. 
