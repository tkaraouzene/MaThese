...
```{r dfbigmmafcall, cache=TRUE, dependson=c("varrunbigmmaf","dfrunbigmmaf","callfunction")}
...
```{r dfctrlbigmmaf, cache=TRUE,  dependson= "varrunbigmmaf"}

df_ctrl_mmaf <- fread("data/call_results/others_novogen_integragen_var_genotype.csv", 
                         sep = "\t", header = TRUE, showProgress = FALSE, verbose = FALSE, data.table = FALSE) %>% 
  select(CHR, POSITION, REF, ALT, one_of(runs_ctrl_bigmmaf)) %>% 
  gather(RUN, CALL, -(CHR:ALT)) %>% 
  filter(CALL %in% c(3, "c"),
         !is.na(REF)) %>%
    distinct(CHR, POSITION, REF, ALT)
