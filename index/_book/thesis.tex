...
% Everything below added by CII
  ...
  \caption[Processus simplifié du contrôle qualité des *reads*]{Processus simplifié du contrôle qualité des *reads* : Pour chacun des graphiques, les *reads* représentés envert sont conservés tandis que ceux en rouge sont filtrés. **A** : Quantité de *reads* bruts générés pour chaque patients au cours de l'étape de séquençage. La médiane des *reads* est représentée en bleue. **C** : Pourcentage pour chaque individu de *reads* s'étant aligné correctement et ne s'étant pas alignés sur le génome de référence. **C** : Distribution pour chaque patient des *reads* compatibles (Comp), non compatibles (Non comp) et orphelins (Orphans). **D** : Présentation pour chaque *reads* du nombre de site auxquels ils s'alignent}\label{fig:readsselection}
  \end{figure}
  ...
  Pour 1 d'entre elle, un cadidat potentiel a pu être mis en évidence
  (\textbf{Figure : }\ref{fig:plotremaininggenes}) et notre équipe
  travaille actuellement sur la caractérisation de ce gène afin de savoir
  s'il peut effectivement expliquer le phénotype MMAF de cette famille.
  
  Pour les 2 familles restantes, l'explication la plus vraissemblable est
  que le variant ait été filtré par l'un de nos six filtres, probablement
