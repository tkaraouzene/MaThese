...
\item
  \textbf{Amplification par génération de \emph{nanoball} d'ADN}
...
amplification par modèle mobile. \textbf{d} : Amplification par
génération de \emph{nanoball} d'ADN.}\label{fig:pictngsampli}
\end{figure}
...
\textbf{a} : Une sonde composée de deux nucléotides connus suivis de
bases dégénérées et de bases universelles est reliée en 5' à un
fluorophore. \textbf{b} : après avoir été hybridé grâce à une ligase,
aux modèle d'ADN à séquencé, les nucléotides spécifiques (les deux
premiers) sont identifiées grâce au fluorophore \textbf{c} : le
fluorophore ainsi qu'une partie bases non spécifiques sont clivés.
\textbf{d} : les étapes 1 et 2 sont ensuite répétées 10 fois permettant
ainsi d'identifier 2 bases tout les 5 nucléotides \textbf{e} : les bases
non spécifiques restantes sont ensuite retirées, un nouveau cycle
reprend avec un décalage de deux nucléotides. Ainsi chaque base est
séquencée deux fois.}\label{fig:pictsblSeq}
\end{figure}
