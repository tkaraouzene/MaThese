...
% Everything below added by CII
  ...
  comme le nombre de \emph{reads} qui, après l'atape d'alignement
  ...
  sondes. Il faut également noté que depuis quelque années, plusieurs
  étude ont remis en cause lk'interet du WES au profis du WGS, nottament
  car le WGS fournit une meilleur couverture sur l'exome que le WES
  ...
  séquencer et à la séléction des kit de capture utilisés (Meienberg et
  ...
  médecine personalisée (Auffray, Chen, \& Hood, 2009). Cependant, pour
  ...
  2009). En effet, les progres techniques des dernières décénies ont rendu
  possible le séquençage de plusieurs millions des \emph{reads} d'ADN en
  un temps reliativement court et à couts raisonable. Ainsi, l'émergence
  du séquençage haut débit et notamment du WGS et du WES a permit de
  réunir une quantité jusqu'à présent inégalé d'information sur les
  variation génétiques, et d'une manière plus générale, sur les gènes et
  leurs fonctions (Mardis, 2008, Bentley (2006)). Cependant, de part leur
  ...
  Redéfinition de ce qu'est \textbf{clairement} un \emph{read}, introduire
  la notion de \emph{paired-end} et \emph{single-end}\\
  présentation d'un fichier fastq (ou autre format de sortie de
  séquenceur)\\
  parler de l'alignement = reconstruction de puzzle\\
  appel des variants = rechercher les différences entre les données de
  séquence et le génome de ref\\
  ...
  sensé représenter la séquence consensus d'une espèce donnée. Par
  ...
  \emph{reads} des l'individus séquencés. Cette étape constitue
  probablement l'étape la plus importante de l'analyse des données issues
  du séquençage haut débit (Flicek \& Birney, 2009). Elle est la base sur
  laquelle reposent l'ensemble des étapes effectuées en aval, notamment
  l'appel des variants (R. Nielsen, Paul, Albrechtsen, \& Song, 2011).
  ...
  (Su et al., 2014). De nombreuse études on cependant montrées de grandes
  ...
  différencié de l'alignement, cependant, les résultats de l'appel étant
  extremement dé\&pendant de l'alignement, il est conseillé d'effectuer
  son appel en tenant compte de l'aligneur choisi (R. Nielsen et al.,
  2011, M. A. DePristo et al. (2011), Lunter \& Goodson (2011)). On
  appelera variants toutes différences de séquence observées entre un
  individu et la séquence de référence utilisée. De nombreux logiciels
  d'appel des variants, ou \emph{caller}, basés sur des algorithmes
  différents ont emmergés ces dernières années pour répondre à cette
  problématique. Parmis les plus connus on note SAMtools (H. Li et al.,
  2009), Genome Analysis Tool Kit - HaplotypeCaller (GATK-HC) (McKenna et
  al., 2010), Freebayes, SOAPindel et tvc . Les quatre premiers cités,
  peuvent être utilisés pour analyser des données provenant de tout type
  de plateforme de séquençage contrairement à TVC qui a été dévoeloppé
  ...
  d'erreur est multi-factorielles et inclus nottament les erreurs de
  ...
  élément, l'appel de variants reste un processus dificille souvent lié à
  ...
  logiciels ne présentent pas les même performances en fonction de ces
  ...
  \caption[Représentation des erreurs d'appel de type IR et AR en fonction de la platforme de séquençage et du logiciel d'appel]{Représentation des erreurs d'appel de type IR et AR en fonction de la platforme de séquençage et du logiciel d'appel d'après [@Hwang2015] : Pour les plateforme Illumina, on peut voir que Freebayes préfère les appels variant-homozygote tandis que GATK-HC et Samtools préfèrent les appels hétérozygotes. Pour la plateforme Ion Proton, les 4 logiciels ont une préférence pour les erreurs de type IR}\label{fig:snperror}
  \end{figure}
  ...
  appelés par au moins 3 d'entre eux. Ce taux chutait drastiquement pour
  ...
  indispensable d'annoter cet ensemble de variant, c'est à dire de les
  replacer dans leur contexte biologique. Cela consiste à ajouter pour
  chaque variant un ensemble d'information qui le caractérise. Ces
  information serviront ensuite d'indicateur afin filtrer ou prioriser un
  variant. Cette dernière étape de l'analyse est elle aussi crutiale
  puisqu'elle permet de réduire le nombre de variant à considérer
  \ldots{}.. On peut généralement distinguer deux niveau d'annotation :
  
  \begin{enumerate}
  \def\labelenumi{\arabic{enumi}.}
  \tightlist
  \item
    Au niveau du variant : Ce niveau d'annotation regroupe l'ensemble des
    informations spécifiques à un variant
  
    \begin{enumerate}
    \def\labelenumii{\alph{enumii}.}
    \tightlist
    \item
      Information issues des résultats du séquençage : la qualité, son
      génotype\\
    \item
      La fréquence du variant dans la population générale : ESP, 1KG, ExAC
      (Lek et al., 2016)
    \item
      Son impact sur l'unité génétique : CADD, SIFT (Kumar, Henikoff, \&
      Ng, 2009), PROVEAN (Y. Choi, Sims, Murphy, Miller, \& Chan, 2012),
      Polyphen2, MutationTASTER (\textbf{Figure :} \ref{fig:vennpred})
    \end{enumerate}
  \end{enumerate}
  ...
  L'annotation au niveau de l'unité génétique consiste à récupérer
  l'ensemble des information disponible sur l'unité (impacté par le
  ...
  extrêmement dépendante du jeux de gène utilisé et peuvent donc varier en
  fonction de celui-ci (D. J. McCarthy et al., 2014,S. Zhao \& Zhang
  (2015)) bien que les gènes CCDS soient bien représentés à la fois par le
  NCBI, Ensembl et UCSC (Pruitt et al., 2009). Malgrès les dizaine de
  milliers d'exomes et de génomes ayant été jusqu'à présent étudiés, notre
  compréhension des mécanismes moléculaire qui sous-tendent la variété
  génomique huamine reste particulièrement limité, et ce particulièrement
  dans le contexte de l'analyse de pathologies génétiques. En pratique, la
  plupart des variants connues pour impliquer une pathologie sont des
  ...
  \section{Materiel \& Methodes}\label{materiel-methodes}
  
  \subsection{Récuération des sonnées ExAC, filtrage et pré
  analyse}\label{recueration-des-sonnees-exac-filtrage-et-pre-analyse}
