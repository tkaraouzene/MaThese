...
% Everything below added by CII
  ...
  la suite cherché à déterminer les taux \emph{de novo} de duplications /
  délétions de ce même locus Afin de voir si cet exces était dû à une
  séléction de l'allèle dupliqué ou au fait que celui-ci était
  effectivement produit plus fréquement que l'allèle délété. Pour ce faire
  nous avons quantifié le taux d'apparition de ces événements génétiques à
  partir d'ADN spermatique. Les spermatozoïdes étant le produits direct de
  la méiose, ils sont donc les reflets d'haplotypes produits \emph{de
  novo}. Pour cela nous avons isolé, par dillutions succèsives l'ADN
  spermatique de trois donneurs de sorte à ce que chaque réaction
  d'amplification amplifie le locus \emph{DPY19L2} d'une seule molécule
  d'ADN (\textbf{Figure} : \ref{fig:dupdelrate}). Ainsi, la probabilité
  qu'un puit contiene une recombinaison (délétion ou duplication) au locus
  \emph{DPY19L2} suivait une distribution hypergéométrique
  (\textbf{Équation} : \eqref{eq:hypergeo}) nous avons donc pu estimer la
  fréquence de duplication / délétion \(\lambda\) (\textbf{Équation} :
  \eqref{eq:lambda})
  ...
  Nous avons pu ainsi établir le taux de délétion \emph{de novo} à 1.8 x
  ...
  dur le site de \emph{DPY19L2}.
  
  \begin{figure}
  
  {\centering \includegraphics[scale=0.35]{figure/globo_du_del_rate} 
